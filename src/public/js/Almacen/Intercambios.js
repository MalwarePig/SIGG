    ...
    $.ajax({
        ...
        success: function (Herramientas) {
            ...
            for (var i = 0; i < Herramientas.length; i++) {
                ...
                var Fecha = Herramientas[i].Fecha;
                //Eliminar variable dentro del For
        ...
        success: function (Herramientas) {
            ...
            for (var i = 0; i < Herramientas.length; i++) {
                ...
                for (var x = 0; x < Arreglo.length; x++) {
                    ...
                    if (x == 5) { //Si termina de registrar datos crear el boton
                        var newCell = newRow.insertCell(6); //CREAR CELDA
                        newCell.innerHTML = '<button id="' + x + '" class="btn btn-danger" name="btn" onclick="EliminarFila(' + (i + 1) + ')"> Eliminar </button>';
                    }
                } //fin de for de columnas
            } //fin de for de filas
        } //Funcion success
...
function EliminarFila(index) {
    ...
    var Producto = Tranformer(tabla.rows[index].cells[0].childNodes[0].nodeValue);
    $.ajax({
